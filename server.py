...
app=FastAPI(title='WaterPulse AI Agent API', version='9.9')
...
@app.get('/api/health')
def health():
    return {'status':'ok','app':'WaterPulse AI Agent','version':'9.9','deployment_layout':'server-self-healing','deepseek_configured':deepseek_configured(),'deepseek_model':deepseek_model(),'field_contract':contract_summary(),'timestamp':now_iso()}
...
def _reference_fallback(message: str, ref: dict[str,Any], intent: str="general") -> str:
    ...
    if material:
        return (f"我可以先分析 {material} 的上游水风险机制和管理重点，不需要先上传 ESG 文件。"
...
def agent_status():
    return {'status':'ok','agent_version':'9.9-executive-decision-report',
    ...
    if action=='show_reference_locations':
        ...
        if not ref.get("material_supported"):
            ...
            return {'session_id':sid,'status':'partial','message':text,
                    'actions':[_action('上传我的企业资料','focus_upload')],'trace':trace,'ai':_ai_meta()}
